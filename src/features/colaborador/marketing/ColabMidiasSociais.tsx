...
import {
  ChevronLeft, ChevronRight, Plus, Video, Image, Target, LayoutGrid, LucideIcon,
} from 'lucide-react';
...
const BRANDS: Record<string, { label: string; handle: string }> = {
  studios: { label: 'Studios', handle: '@purepilatesbr' },
  academy: { label: 'Academy', handle: '@purepilatesacademy' },
  franchising: { label: 'Franchising', handle: '@purepilates.franchising' },
};

const TAG_LABELS: Record<string, string> = {
  reels: 'Reels', desafio_semana: 'Desafio da Semana', carrossel: 'Carrossel', estatico: 'Estático',
};
const TAG_COLORS: Record<string, string> = {
  reels: 'bg-purple-500 text-white', desafio_semana: 'bg-red-500 text-white',
  carrossel: 'bg-teal-500 text-white', estatico: 'bg-blue-500 text-white',
};
const TAG_LEGEND_COLORS: Record<string, string> = {
  reels: 'bg-purple-500', desafio_semana: 'bg-red-500', carrossel: 'bg-teal-500', estatico: 'bg-blue-500',
};
const TAG_ICONS: Record<string, LucideIcon> = {
  reels: Video, desafio_semana: Target, carrossel: LayoutGrid, estatico: Image,
};

const ColabMidiasSociais = () => {
  const { brand: brandParam } = useParams<{ brand: string }>();
  const brand = brandParam && BRANDS[brandParam] ? brandParam : 'studios';
  ...
  const fetchContent = async () => {
    ...
    if (error) { console.error('Error fetching brand content:', error); return; }
    setContent((data ?? []) as SocialMediaContent[]);
  };

  useEffect(() => { fetchContent(); /* eslint-disable-next-line */ }, [brand]);

  useEffect(() => {
    let t: ReturnType<typeof setTimeout>;
    const channel = supabase
      .channel(`colab-midias-${brand}`)
      .on('postgres_changes', { event: '*', schema: 'public', table: 'social_media_content' }, () => {
        clearTimeout(t); t = setTimeout(() => fetchContent(), 1200);
      })
      .subscribe();
  ...
  const handleEditClick = () => { setIsDetailsDialogOpen(false); setIsEditDialogOpen(true); };
    ...
    <MainLayout>
      <div className="max-w-6xl mx-auto">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6 sm:mb-8">
          <div>
            <h1 className="text-xl sm:text-3xl font-bold text-foreground">Mídias Sociais — {brandInfo.label}</h1>
            <p className="text-sm sm:text-base text-muted-foreground mt-1">
              Calendário de conteúdos — <span className="font-medium">{brandInfo.handle}</span>
            </p>
          </div>
          {(isColaborador || isAdmin) && (
            <Button onClick={() => { setSelectedDate(null); setIsCreateDialogOpen(true); }} size="sm" className="w-full sm:w-auto">
...
export default ColabMidiasSociais;

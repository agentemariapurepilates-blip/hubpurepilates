...
function preencher(troca: Partial<Record<'valor' | 'qtd' | 'email', string>> = {}) {
  ...
  digitar(/quantidade de professores/i, troca.qtd ?? '3');
...
describe('Solicitar Verba para Novos Professores', () => {
  ...
  it('valor com centavos é lido em reais e o pedido sai', async () => {
    ...
    expect(invoke.mock.calls[0][1].body).toMatchObject({ valor_verba: 9000, qtd_professores: 3 });
  });
  ...
  it('com um campo recusado, diz o motivo depois do aceite em vez de só travar o botão', () => {
    ...
    expect(screen.queryByText(/quantidade de professores precisa/i)).toBeNull();

    fireEvent.click(screen.getByRole('checkbox'));
    expect(botao().disabled).toBe(true);
    expect(screen.getByText(/quantidade de professores precisa ser de 1 a 50/i)).toBeTruthy();
  });
});
